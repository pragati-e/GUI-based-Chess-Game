 ...
 * File Name: Main.java
 * 
 * Description: All the GUI elements are specified, initialized, and utilized. It is derived from the Java Swing Library's JFrame Class.
 */
...
public class Main extends JFrame implements MouseListener {
	...
	private Main() {
		...
		controlPanel.setBorder(BorderFactory.createTitledBorder(null, "Statistics", TitledBorder.TOP,
                                                                TitledBorder.CENTER, new Font("Lucida Calligraphy", 
                                                                Font.PLAIN, 20), Color.ORANGE));
		
		//using the control panel to create the player box 
		WhitePlayer = new JPanel();
		WhitePlayer.setBorder(BorderFactory.createTitledBorder(null, "White Player", TitledBorder.TOP, 
                                                               TitledBorder.CENTER, new Font("times new roman",
                                                               Font.BOLD,18), Color.RED));
		...
		//if player found inactive for long
		temp=new JPanel(){
			private static final long serialVersionUID = 1L;
	...
	//retrieving the Black King or White King
	...
	//clearing the markings of possible destination cells
    ...
    //outlining the cell formation to show possible movements
    ...
    //checking for danger for king for a particular move
    ...
    //removing moves that might endanger the King
    ...
    //filtering the alternative moves when the current player's king is in check 
    ...
    //determining if the King is check-mate and game over if true
	...
	private void gameend() {
    	...
    	if(chance==0) {	White.updateGamesWon();
			White.Update_Player();
			winner=White.name();
		}
		else {
    ...
    //abtract functions of parent class
	@Override
